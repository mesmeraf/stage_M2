...
\caption{\textbf{Niveau d'expression des gènes consécutifs dans les opérons.} Les données proviennent de l'expérience de choc thermique pendant 15 minutes. L'$I_{expr}$ en abscisses est l'indice d'expression du gène 1 par rapport au gène 2 $(gene1 - gene2) / (gene1 + gene2)$, borné sur [-1,1]. Les valeurs positives indiquent un niveau d'expression plus élevé du gène 1, les valeurs négatives l'inverse. Les éléments de couleur rouge se réfèrent aux gènes flanquant une BIME, ceux en bleu aux gènes sans BIME. Les histogrammes indiquent la densité pour chacune des valeurs de l'$I_{expr}$ par rapport au type d'élément transcriptionnel présent dans la RIG, violet pour les promoteurs, orange pour les terminateurs et incolore en leur absence. La couleur de leur contour souligne à titre indicatif la présence d'une BIME. Les courbes de densité montrent la tendance de l'$I_{exp}$ par rapport aux BIME, la comparaison entre les 2 distributions est réalisée à l'aide d'un test de Wilcoxon.}
\label{fig:expression_operon}
\end{figure}

L'analyse menée sur les données d'expression des bactéries soumises à un choc thermique pendant 15 minutes est représentative des deux autres jeux de données (résultats non montrés). La comparaison des distributions en présence et absence de BIME est effectuée à l'aide d'un test de Wilcoxon car toutes les distributions ne suivent pas une loi normale, l'hypothèse nulle étant la similitude des distributions. L'absence de BIME montre une répartition symétrique de l'expression des deux gènes centrée sur une expression similaire sauf en présence de promoteurs/terminateurs dans la RIG, la tendance étant vers une expression plus importante du gène 2 (\autoref{fig:expression_operon} - (b,e,h)). Ceci s'explique par la présence de promoteurs favorisant la transcription du gène 2 visibles sur les valeurs négatives de l'$I_{expr}$.
L'absence de promoteur/terminateur et la présence de BIME montre un niveau d'expression plus élevé du gène 1 par rapport au gène 2 et cela indépendamment de la taille de la RIG (\autoref{fig:expression_operon} - (a,d); p-values: 0.047, 0.00035). La présence de promoteur/terminateur et de BIME dans les RIG inférieures à 150 pb permet de distinguer nettement trois effets, expression augmentée du gène 1, expression augmentée du gène 2 et même niveau d'expression sans pour autant observer de différence significative des distributions en présence et absence de BIME (\autoref{fig:expression_operon} - (b); p-value: 0.303). Cela peut être dû à un effet compensatoire de la BIME qui pourrait révéler un rôle de stabilisation de la partie 5' du transcrit face au dégradosome. Avec l'augmentation de la taille de la RIG, nous perdons cette distribution tri-modale, mais nous observons un décalage de la courbe en présence de BIME vers la droite indiquant un niveau plus important d'expression du gène 1 (\autoref{fig:expression_operon} - (e); p-value: 0.047). Cette tendance est peut être due à la présence combinée des terminateurs et des BIME pouvant jouer le même rôle puisque nous observons un regroupement de ces derniers vers la droite (contours rouges et remplissage orange) par rapport à ceux sans BIME étalés entre 0 et 1 (contours bleus et remplissage orange) ou à un effet de stabilisation du transcrit en 5' par la BIME.
