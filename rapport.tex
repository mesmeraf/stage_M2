...
\newacronym{sam}{\texttt{SAM}}{Sequence Alignment Map}
\newacronym{bam}{\texttt{BAM}}{Binary Alignment Map}
...
% efface la numérotation de page pour le sommaire
\cleardoublepage
\pagenumbering{gobble}
	
\tableofcontents

% recommence la numérotation
\cleardoublepage
...
\addcontentsline{toc}{section}{Caractéristiques des REP et organisations en BIME}
...
\addcontentsline{toc}{section}{Propriétés associées aux REP}
...
\begin{SCfigure}[][h!]
\fbox{\includegraphics[scale=0.63]{figures/malEF_rep.png}
\caption{\textbf{Structure ARN des REP au sein de l’opéron \textit{malEFG}.} Y, Z\textsuperscript{2} et Y indiquent la séquence des REP dans l'espace inter-génique de \textit{malE-malF}. Bien que Y et Z\textsuperscript{2} puissent former des structures tige-boucles par elles mêmes, elles s'apparient ensemble pour former une région étendue en grande partie à double brin (70\% des nucléotides sont appariés). La séquence affichée provient du génome d'\textit{E. coli} K12. Les codons STOP de  \textit{malE} et START de \textit{malF} sont encadrés. SD représente la séquence Shine–Dalgarno nécessaire à l'initiation de la traduction de \textit{malF}. La région REP-stabilized RNA (RSR) indique l'extrémité 3′ du messager \textit{malE} mature, qui s'étend de 3 à 9 nucléotides depuis la base de la tige-boucle formée par Y et Z\textsuperscript{2} qui stabilise cette partie de l'opéron codant pour la protéine affine du transporteur ABC. \citep{Khemici2004}.}
\label{fig:malEF_rep}}
\end{SCfigure}

\section*{ARN messagers chez \textit{E. coli}}
\addcontentsline{toc}{section}{ARN messagers chez \textit{E. coli}}

\subsection*{Unités de transcription}
\addcontentsline{toc}{subsection}{Unités de transcription}
...
\addcontentsline{toc}{subsection}{Stabilité des ARNm}
...
\addcontentsline{toc}{subsection}{Terminaison de la transcription}
...
\addcontentsline{toc}{chapter}{Matériel \& Méthodes}
...
\addcontentsline{toc}{section}{Jeux de données}
...
\addcontentsline{toc}{section}{Fichiers d'annotations}
...
\addcontentsline{toc}{section}{Alignement des reads}
...
\addcontentsline{toc}{section}{Intégration des données}
...
\addcontentsline{toc}{section}{Visualisation du mapping}
...
\addcontentsline{toc}{section}{Détermination des transcrits}
...
\addcontentsline{toc}{section}{Segmentation}
...
\addcontentsline{toc}{section}{Reconstruction des séquences ancêtres et structures secondaires}
...
\addcontentsline{toc}{section}{Couverture du génome}
...
\addcontentsline{toc}{section}{BIME et unités de transcription}
...
\addcontentsline{toc}{section}{Effet global dans les unités de transcription}
...
\begin{tabular}{|l|c|c|c|c|c|c|}
  \hline
  \rowcolor{Gray} Jeux de données & $\frac{Nb.~operons~avec~genes~DE}{Nb.~d'operons~exprimes}$ \scriptsize{\dag} & \begin{tabular}{c}RIG avec\\Terminateurs\end{tabular} & \begin{tabular}{c}Terminateurs\\avant BIME\end{tabular} & \begin{tabular}{c}Terminateurs\\sur BIME\end{tabular}\\ 
  \hline
  GSE61327\_ALE & 17/32 & 7/17 & 2/7 & 3/7\\  
  \hline
  HS-15min & 8/20& 1/8 & 0/1 & 0/1\\ 
  \hline
  M-P4h & 12/30& 4/12 & 0/4 & 2/4\\
 \hline
\end{tabular}}
...
\addcontentsline{toc}{section}{Localisation des variations de taux d'expression}
...
\addcontentsline{toc}{subsection}{Approche par segmentation}
...
\addcontentsline{toc}{section}{Biais de séquençage des REP}
...
\addcontentsline{toc}{section}{Expression des gènes dans les opérons et présence de BIME}
...
Afin d'aller plus loin dans l'analyse, il serait intéressant de réaliser 2 types d'expérience de RNA-Seq. La 1\up{ère} élaborée avec une souche sauvage et une souche pour laquelle serait inactivés les gènes codant pour des protéines impliquées dans la dégradation des ARNm. Elle nous permettrait d'affecter la dégradation des ARNm et vérifier si les gènes en aval de la BIME dans les opérons dont les taux d'expression sont plus faibles que ceux en amont dans la condition sauvage ne seraient plus différentiellement exprimés dans la condition perturbée. La 2\up{nde} serait une expérience bloquant la transcription avec un échantillonnage dans le temps pour suivre la dégradation des transcrits.
...
% fin de la numérotation des pages
\cleardoublepage
\pagenumbering{gobble}
...
\begin{tabular}{|l|c|c|c|c|c|c|c|}
  \hline
  \rowcolor{Gray}\begin{tabular}{c}Couples de gènes\\de l'opéron\end{tabular} & \begin{tabular}{c}Gène 1\\RPKM\end{tabular} & \begin{tabular}{c}Gène 2\\RPKM\end{tabular}  & \begin{tabular}{c}Souches\\dans le\\cluster \end{tabular} & \begin{tabular}{c}Énergie de\\la structure\\secondaire\end{tabular} & \begin{tabular}{c}$\Delta$ énergie\\sequence\\ancestrale\end{tabular} & \begin{tabular}{c}Famille\\de\\BIME\end{tabular}\\
  \hline
...
\caption{\textbf{Résultats de la reconstruction des séquences ancêtres et de l'étude des structures secondaires pour les BIME des couples de gènes consécutifs avec différence d'expression dans les opérons.}}
\end{table}
...
\begin{figure}[h!]
\centering
\subfigure[]{\label{fig:seg_GSE} \includegraphics[scale=0.47]{figures/supData/seg_DPPB_DPPA_GSE.png}}
\subfigure[]{\label{fig:seg_MP} \includegraphics[scale=0.47]{figures/supData/noSeg_DPPB_DPPA_MP4h.png}}
